  ...
  testScript = # python
    ''
      ...
      with subtest("a labelled service is published while it runs"):
          ...
          routing_lines = routing.splitlines()
          assert routing_lines[0] == "# Generated by systemd_traefik_configuration_provider, do not edit.", \
              f"the routing file does not start with the provider's marker, so it would never be pruned: {routing}"
          assert routing_lines[1] == "http:", \
