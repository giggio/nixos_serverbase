...
let
  ...
  ubootOrangePi4Pro = pkgs.buildUBoot {
    ...
    postConfigure = ''
      # Raise the autoboot countdown from the vendor default (2s) to 5s. This is a recovery/debugging affordance: 2 seconds is
      ...
      echo "Setting CONFIG_BOOTDELAY=5..."
      if [ -x ./scripts/config ]; then
        ./scripts/config --set-val BOOTDELAY 5
      else
        sed -i '/CONFIG_BOOTDELAY/d' .config
        echo "CONFIG_BOOTDELAY=5" >> .config
      fi
      make olddefconfig
    ...
    postPatch = ''
      ...
      # Allwinner hardcodes `bootdelay = 1;` as the first statement of __abortboot() (common/autoboot.c), which discards both
      # CONFIG_BOOTDELAY and the `bootdelay` environment variable. That is why `printenv` reports bootdelay=5 while the board
      # still counts down from 1 (printing "1" then "0", so it looks like ~2 seconds). Delete the line so the configured value
      # is actually honoured. Without this there is no way to reach the "=>" prompt except by mashing a key at exactly the right
      # moment — which matters, because that prompt is this board's only recovery mechanism.
      sed -i '/^[[:space:]]*bootdelay = 1;[[:space:]]*$/d' common/autoboot.c
      if grep -qE '^[[:space:]]*bootdelay = 1;' common/autoboot.c; then
        echo "FATAL: the bootdelay hardcode in common/autoboot.c is still present"; exit 1
      fi
