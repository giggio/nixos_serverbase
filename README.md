...
Clone this repo, and add the secret file to `/home/giggio/.config/nixos-secrets/server.agekey`. Its directory is used as a flake input.
In the future I might evolve it to be in a Git repository, but this is good enough for now.

Build it with:

```bash
nix build .?submodules=1#vbox
```
