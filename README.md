...
generate custom targets, so you could run `out/nix/vm/run-gmktec1dev-vm` to build the
QEMU run script, or `out/nix/img/pi4.img.zst` to build the Raspberry Pi 4 image,
...
3. Start a new VM with `make start_new_gmktec1dev`, this will connect to the
   serial port of the machine.
4. After the machine boots, you can also connect to it via SSH on localhost,
   port 2222.
...
If you have problems, inspect the virtual machine script after you import it,
make sure it matches your hardware.
...
to test the .iso installer. This is much slower as the ISO creation and then the
copy to the QEMU disk is slow.
There is no similar way to test the .img installer.
